...
object main extends App {
  ...
  /**
   * Cambia el estado de una lista de posiciones dentro del tablero de forma concurrente.
   * @param tablero Matriz concurrente de enteros a modificar.
   * @param lista_pos Lista de posiciones (fila, columna) a modificar.
   * @param estado Nuevo estado para las posiciones de la lista.
   * @return Tablero con todas las columnas de lista_pos modificadas.
   */
  def cambiar_estado_multipos(tablero: ParVector[ParSeq[Int]], lista_pos: List[List[Int]], estado: Int): ParVector[ParSeq[Int]] = {
    // Cada casilla comprueba si su posición en el tablero está contenida en la lista de objetivos, si está modifica su estado.
    // zipWithIndex permite usar map con índices y no solo con valores. (Hilos conscientes de su posición (CUDA))
      ...
      bloque match { // Comprobamos si es una bomba o no
        ...
        case _ => // Si no
          ...
          if (iguales.length > 2) { // Si se forma un grupo de 3 o más bloques iguales
            par_to_list(cambiar_estado_multipos(list_to_par(tablero), iguales, 0)) // Eliminamos de forma concurrente todos los bloques del grupo
          }
  ...
  def tablero_vacio(tablero: ParVector[ParSeq[Int]]): Boolean = {
    // Filtramos las posiciones que no sean 0, si no hay ninguna, el tablero está vacío
  ...
  def actualizar_puntuacion(t1: List[List[Int]], t2: List[List[Int]], puntuacion:Int, bloque_pulsado: List[Int]): Int = {
    /**
     * Función auxiliar para calcular la diferencia entre dos tableros de forma concurrente.
     * Cada hilo comprueba una celda en paralelo en los dos tableros.
     * @param t1 Tablero original.
     * @param t2 Tablero modificado.
     * @return Diferencia entre los dos tableros.
     */
  ...
  /**
   * Comprueba el numero de vecinos de cada posicion de la lista de posiciones y actualiza el valor del mejor grupo y mejor posicion si
   * encuentra una mejor a la actual.
   * @param posiciones posiciones para evaluar
   * @param mejor_grupo mejor puntuacion encontrada
   * @param mejor_pos mejor grupo encontrado
   * @param f funcion anonima para comparar elementos
   * @return
   */
  @tailrec
  ...
  /**
   * Crea un hilo con cada posicion del tablero para que, de forma concurrente se recorra el tablero buscando cual es
   * la mejor opcion para pulsar. Si ninguna posicion suma puntos, devuelve como puntuacion -1
   * @param tablero tablero actual
   * @return coordenada a pulsar en la siguiente iteracion de la inteligencia Artificial
   */
